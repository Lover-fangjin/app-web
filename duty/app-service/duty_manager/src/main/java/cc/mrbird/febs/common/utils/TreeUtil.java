...
public class TreeUtil {
    ...
    public static <T> List<BusinessOrgTree<T>> buildBusinessOrgTree(List<BusinessOrgTree<T>> nodes) {
        if (nodes == null) {
            return null;
        }
        List<BusinessOrgTree<T>> result = new ArrayList<>();
        nodes.forEach(children -> {
            String pid = children.getParentId();
            if (pid == null || "0".equals(pid)) {
                result.add(children);
