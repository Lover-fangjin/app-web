<template>
    <div class="site-list" v-if="features.length > 0">
        <ul v-for="item in features" :key="item.id" @click="location(item)" @mouseover="onMouseOver(item.name)" @mouseleave="onMouseLeave()">
...
export default {
    ...
    methods: {
        location(item){
            ...
            lookAndFlyPoint(parseFloat(item.lon), parseFloat(item.lat), 0, 3000);
        },
        onMouseOver(_name){
            let me = this;
