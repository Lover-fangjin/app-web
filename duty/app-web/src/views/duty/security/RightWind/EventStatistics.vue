                    ...
                    <el-col :span="24">
                      ...
                      <div class="demo-image__preview" v-if="item.imgs.length > 0">
                        <el-image  v-for="(img, index) in item.imgs" :key="index"
                          ...
                          :src="img" 
                          :preview-src-list="formatImgs(item.imgs, index)">
                        </el-image>
                      </div>
                    </el-col>
                  </el-row>
...
import { lookAndFlyEntityOfName, lookAndFlyPoint, removeEntityByType, drawEntityPoiSingle } from '@/util/common3d';
...
export default {
  ...
  methods: {
    filterActivities( _eventlist){
      ...
      // debugger
      me.activities = [];
      _eventlist.filter(event => {
        if(me.radio === event.eventClass){
          let _e = Object.assign({}, event);
          _e.imgs = _e.imgs.map(img => { return config.services +  img;});
          me.activities.push(_e)
        }
      });
      // let _types = [];
      // if(me.checked){
    ...
    location(item){
      let me = this, 
      lon = item.lon === null ? 0 : parseFloat(item.lon), 
      lat = item.lat === null ? 0 : parseFloat(item.lat),
      _viewer = config.zInst1.viewer;
      lookAndFlyPoint(lon, lat, 0, 1000, -90);
      ...
      let info = {
        ...
        imgs: me.people.imgs,
        type:  me.people.type,
        group: me.people.group
      }
      console.log('info: ', info);

      removeEntityByType(_viewer, me.people.type, () => { 
        ...
        me.handler.setInputAction(function onMouseClick(movement) {
          ...
          if(Cesium.defined(entity) && entity.hasOwnProperty('type') && entity.type === me.people.type){
    ...
    formatImgs(arrs, index = 0) {
      return arrs.slice(index).concat(arrs.slice(0, index));
    }
  },
  destroyed () {
    this.checked = [];
