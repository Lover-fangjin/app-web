...
<script>
import Navigation from '../ncov/Navigation.vue';
  ...
  mounted () {
    ...
    this._eChart.on("mousemove", param => {
      ...
      if (self.areaType !== "world") {
        ...
        this.$store.commit('updateMousemoveArea', param.name);
      }
    });
  },
  methods: {
    ...
    toProvince (province) {
      ...
      if (!name)
        return;
      this.$router
        .push({ name: "ncov", params: { area: name } })
        .catch(err => {
          err;
        });
      this.$store.commit('updateCurrentArea', name);
    },
    updateChart () {
      this.$ds.loadSubArea( this.currentArea ).then(data => {

        console.log("leftCharts: ", data)
