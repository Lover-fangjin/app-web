...
export let drawEntityPoi2 = (viewer, name, infos) => {
    let entities = [];
    var circleSize = 1000;
        ...
        infos.map(info => {
            ...
            entities = viewer.entities.add({
                ...
                label: { //文字标签
                    ...
                    fillColor: new Cesium.Color(31/255, 166/255, 231/255, 1),
                ...
                //         return s2;
                //     },false),
                //     material:"../assets/img/circle2.png",
 ...
 * @param {*} type 实体类型
 */
export let removeEntityByType = (viewer, type) => {
    let entities = viewer.entities.values.filter(item => { if(item.type === type) return item})
    entities.map( entity => { viewer.entities.remove(entity) })
}
 ...
 * @param {*} name 实体名称
 */
export let showEntityByName = (viewer, name) => {
    let entities = viewer.entities.values.filter(item => { if(item.name === name) return item})
 ...
 * @param {*} name 实体名称
 */
export let hidEntityByName = (viewer, name) => {
    let entities = viewer.entities.values.filter(item => { if(item.name === name) return item})
