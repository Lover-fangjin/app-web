...
export let unHighlight = (_viewer) => {
...
export let highlight = (_viewer, _id) => {
    unHighlight(_viewer);
    let entities = _viewer.entities.values;
    entities.map(item => {
        if(item.id == _id && item.hasOwnProperty('imgs')){
 ...
 */
export let creatHtmlElement = (_viewer, element, position, arr, flog) => {
    var scratch = new Cesium.Cartesian2(); //cesium二维笛卡尔 笛卡尔二维坐标系就是我们熟知的而二维坐标系；三维也如此
    var scene=_viewer.scene, camera=_viewer.camera;
